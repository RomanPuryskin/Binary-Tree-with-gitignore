...
//-----------Деструктор---------------//
BinaryTree::~BinaryTree() { DestroyTree(m_root); }
...
void BinaryTree::DestroyTree() { DestroyTree(m_root); }
//------------------------------------//
...
Node *BinaryTree::copyTree(Node *root) {
  ...
  if (root->GetLeft() != nullptr)
    ...
    root_copy->SetLeft(nullptr);

  if (root->GetRight() != nullptr)
    root_copy->SetRight(copyTree(root->GetRight()));
  else
    root_copy->SetRight(nullptr);
  return root_copy;
}
//---------------------------------------------------------//
...
bool BinaryTree::isEmpty() { return isEmpty(m_root); }
//--------------------------------------------------------------//
...
//------------Получение корня-------------------//
Node *BinaryTree::getRoot() { return m_root; }
//----------------------------------------------//
...
int BinaryTree::Count() const { return Count(m_root); }
//---------------------------------------------------//
...
void BinaryTree::PrintLeaves() { PrintLeaves(m_root); }
//-------------------------------------------------------//
...
int BinaryTree::height() const { return height(m_root); }
//-------------------------------------------------------------//
...
int BinaryTree::sumKeys() { return sumKeys(m_root); }
//-------------------------------------------------------------//

//------------Получение уровня вершины по ключу ---------------//

Node *BinaryTree::searchNode(Node *root, int key) {
...
bool BinaryTree::balancedTree(Node *root) {
  ...
  if (abs(leftHeight - rightHeight) <= 1 && balancedTree(root->GetLeft()) &&
      balancedTree(root->GetRight()))
    return true;
  return false;
}

bool BinaryTree::balancedTree() { return balancedTree(m_root); }
//--------------------------------------------------//
